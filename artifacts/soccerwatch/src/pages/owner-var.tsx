...
const kinds: Array<{ value: VarMarkKind; color: string }> = [
  { value: "goal", color: "border-turf/40 bg-turf/10 text-turf" },
  { value: "foul", color: "border-line bg-raised text-muted-text" },
  { value: "offside", color: "border-line bg-raised text-muted-text" },
  { value: "other", color: "border-violet/40 bg-violet/10 text-violet" },
];
...
export default function OwnerVar() {
  ...
  if (authLoading || statusState.loading) {
    return <main className="flex min-h-[100dvh] flex-1 items-center justify-center bg-void px-6 text-sm text-muted-foreground">{copy.loading}</main>;
  }

  return (
    <main dir={isArabic ? "rtl" : "ltr"} className="flex min-h-[100dvh] flex-1 flex-col overflow-y-auto bg-void text-text">
      <header className="flex items-center gap-3 border-b border-line px-4 py-4">
        <button type="button" onClick={() => setLocation("/owner?tab=footage")} aria-label={copy.back} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-line bg-raised">
          <ArrowLeft className="h-4 w-4 rtl:rotate-180" />
        </button>
        <div className="min-w-0">
          <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-turf">VAR</p>
          <h1 className="truncate text-lg font-bold">{statusState.status?.fieldName ?? "VAR"}</h1>
        </div>
      </header>

      {statusState.error ? (
        <section className="m-4 rounded-2xl border border-line bg-surface p-4 text-sm">
          <div className="flex items-start gap-2"><CircleAlert className="mt-0.5 h-4 w-4 text-floodlight" /><p className="flex-1 text-text">{copy.error}</p><button type="button" onClick={() => void statusState.refresh()} className="text-xs font-bold underline text-muted-text">{copy.retry}</button></div>
        </section>
      ) : matchOver ? (
        <section className="m-4 rounded-2xl border border-line bg-surface p-4">
        ...
        <section className="m-4 rounded-2xl border border-line bg-surface p-4 text-sm">
          <p className="font-bold">{copy.closed}</p><p className="mt-1 text-xs text-muted-text">{copy.closedDesc}</p>
        </section>
      ) : null}
      ...
      <section className="mx-3 mb-4 rounded-2xl border border-line bg-surface p-4">
        ...
        <label className="mt-3 block text-xs text-muted-foreground">{copy.note}<textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={500} placeholder={copy.notePlaceholder} className="mt-1 min-h-16 w-full resize-none rounded-xl border border-line bg-void/20 p-3 text-sm text-text outline-none focus:border-turf" /></label>
      ...
      <section className="mx-3 mb-8 rounded-2xl border border-line bg-raised p-4">
        ...
        <div className="mt-3 space-y-2">
          {marks.length === 0 ? <p className="text-xs text-muted-foreground">{copy.noMarks}</p> : marks.map((mark) => (
            <div key={mark.id} className="flex items-center gap-2 rounded-xl border border-line bg-void/15 p-2.5">
